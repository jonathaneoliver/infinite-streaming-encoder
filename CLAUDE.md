...
`phase_mezzanine` short-circuits on the `.done` sidecar, so the handoff needed no
new mechanism — but the workflow is told not to schedule the activity at all
(`mezz_ready`). Leaving it in place worked and was still wrong twice over: it
re-ran, on whatever box drew it, the check this process had just done — 2s of
dispatch and a cross-LAN HEAD against the 300ms it takes to build the mezzanine
here — and it attributed the row to a machine that had done no work.

It is a plan key like `host_package` and read the same way (`plan.get`), so a
worker on older code still schedules the activity and `phase_mezzanine` still
guards itself — this removes a redundant check, never the only one. That farm is
also the only reason `_SELF_RUN_STAGES` still carries `"mezzanine"`: a box
mid-rolling-update schedules the activity anyway, and its SCHEDULED event would
walk a finished row back to `queued`.

**A cache hit names no box at all.** `_build_mezzanine_on_host` calls
`_emit_self_run_host` because this box really did the work; the cache-hit branch
deliberately does not, because nothing built that mezzanine — naming the worker
that drew the old no-op guard read as "ubuntu built the mezzanine" when ubuntu
had done a HEAD and returned. It emits `ENCODER-REUSED` instead, the same marker
a resumed chunk uses and already styled as a distinct neutral. The
`emit_stage(done)` beside it is load-bearing rather than cosmetic now: with no
activity scheduled, nothing else will ever move that row off pending.
...
- **The `_xs` output tag is an interface with go-live, and NOTHING in this repo reads it.** `deriveOutputTag` emits it for the flexible base (no pinned `segment_duration`); go-live keys off it to decide which outputs it may repackage into 1s/2s/6s. Every use here is concatenation into a directory name plus `ValidPathSegment` and the form's collision check — no code branches on the value — so renaming it, or letting a ladder derive it by accident, breaks nothing any test or encode here would notice. `TestFlexibleBaseStillDerivesXs`'s failure message is the only in-tree statement of the consequence. It is also a CLAIM: `xs` asserts the encode's VBV keeps `peak/avg = maxrate% + bufsize/T` inside our 1.25x live figure **at T=1s**, which is what makes it safe to re-chop (that figure is OURS — Apple's spec bounds peak/avg only for VOD, at 2x (rule 1.30); its live 125% rule (1.29) bounds measured peak against the DECLARED `BANDWIDTH`, not against the average) — but the tag is derived from the segment being unset and nothing else, so a loose-VBV ladder that pins no segment asserts a re-choppability its numbers do not support. Unenforced. See [`docs/ladders-and-delivery.md`](docs/ladders-and-delivery.md) § "What the tags MEAN".
- **A ladder's `extra_args` can override what the encoder hardcodes — and the learned-speed model cannot see that it did.** `extra_args` are appended AFTER the codec args and ffmpeg takes the LAST occurrence of an option, so `"av1": "-preset 4 -pix_fmt yuv420p10le"` really does change preset and bit depth (verified, and the only way to reach either from a ladder; scaler flags are NOT reachable, because the scale filter is built inside `burnin.build_filter` and a second `-vf` would replace the whole chain including burn-in and tpad). But `speedKey` is `machine:codec:height:passes:preset:fps` and reads `preset` from `Rung.preset` — `"medium"` for every av1 rung, since `_codec_specific_args` hardcodes `-preset 6` for av1 and the override arrives later in the argv. So a preset-4 or preset-2 run writes its samples onto the SAME key as preset 6 (measured 1.60x and 4.12x slower), and `dynamicChunkSecondsAt` then plans chunks from a blend of presets. #314's shape exactly: one rule, copied somewhere that cannot see the override. `scripts/purge_speed_keys.py` clears a contaminated key, server stopped. Measurements and method in [`docs/ladders-and-delivery.md`](docs/ladders-and-delivery.md) § "Measured: what bit depth and preset actually cost".
- **`-maxrate`/`-bufsize` never reach av1**, so a ladder's VBV fields shape h264 and hevc only: libsvtav1 rejects a peak cap outside CRF mode ("Max Bitrate only supported with CRF mode") and `encode_variants` omits them for that codec. A ladder that documents its VBV split is describing two of its three codecs.
- **The burn-in MODE rides inside the `BURNIN` env value, not beside it.** `BURNIN` is `"true"` / `"false"` / `"light"` (`JobConfig.BurninEnv` → the SFN input's `burnin` field → every variant job's `containerOverrides`), and `cli_phase._burnin_mode` reads the third spelling. That is deliberate: a `BURNIN_MODE` env would mean editing `definition.json.tpl`, both Maps' `ItemSelector`s and the job definitions, so the feature could not ship without a `make deploy` — and a worker on older code reads `"light"` as truthy and draws the FULL overlay, which is the degradation you want. Locally the same choice travels as `--burnin-mode light` (Go → `cli_local`/`cli_local_dist`) and as the `burnin_mode` plan key (orchestrator → `temporal_worker`, read with `plan.get` so an older worker ignores it). **Anything unrecognised means full on every path**, in both languages: an unasked-for legible overlay is recoverable, an encode silently missing the label it is identified BY is not. Light draws ONE static `JEO_<rung>_<kbps>k` label because the timecode row re-renders every frame and so costs bits continuously, where a static label is inter-predicted for free after each keyframe — and it does NOT make an encode VMAF-eligible, because any drawn pixels bias a full-reference metric. Pinned from both ends: `scripts/test_burnin_light.py` and `internal/encode/burnin_mode_test.go`.
...
- **Dynamic chunking is per-variant on BOTH targets now (#362), and the sizing lives in Go on both.** It used to be cloud-only — not because local had no model but because `dynamicChunkSecondsAt` hardcoded `speeds.Speed("graviton", …)`, so "dynamic" on a local target fell through to `localChunkSeconds`'s fixed 2×segment. `speedKey` has always carried the machine, so the farm's boxes were being learned the whole time; only the caller was cloud-shaped. The two selectors differ in exactly one term and share `quantizeChunkSeconds`: cloud sizes for graviton, local for `LocalSpeedSlowest` — the **slowest** learned box, not the average `LocalSpeedN` predicts wall time with, because chunks go onto one queue every worker polls and "~4 minutes of work" is a BOUND on whoever takes one. Three silent-failure seams, all pinned: `--variant-chunk CODEC/LABEL:SECONDS` (Go → `_parse_variant_chunks`, same shape as `--vmaf-estimate` and parsed the same way — `rsplit(":", 1)` then `split("/", 1)`, so a slash in a label survives and a colon in one would not); the `chunks_by_variant` plan key (orchestrator → workflow, pinned by `test_dist_stage_state`); and the uniform `chunks` list, which **must stay** — it is what a rung Go did not size uses, and what a worker still on pre-#362 code reads. Go and Python resolve rungs separately (`resolveRungs` vs `select_rungs`), so an unmatched label MUST degrade to that uniform plan rather than fail. No whole-job budget pass locally: #316 rations against Step Functions' 25,000-event history and Temporal's limits are ~3x larger.
