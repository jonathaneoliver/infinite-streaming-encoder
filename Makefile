...
check:                ## run the same static checks CI runs (gofmt/vet/build/test, staticcheck, govulncheck, tofu fmt, py compile, page JS)
	...
	printf '  py chunkjoin   '; \
	if out=$$(python3 scripts/test_chunk_joins.py 2>&1); then echo "ok"; \
	else echo "FAIL"; echo "$$out" | sed 's/^/                 /'; fail=1; fi; \
	printf '  py chunkgrid   '; \
	...
	printf '  py vmafperiod  '; \
	if out=$$(python3 scripts/test_vmaf_periodicity.py 2>&1); then echo "ok"; \
	else echo "FAIL"; echo "$$out" | sed 's/^/                 /'; fail=1; fi; \
	printf '  py diststate   '; \
	...
	printf '  py mezzcfr     '; \
	if out=$$(python3 scripts/test_mezz_cfr.py 2>&1); then echo "ok"; \
	else echo "FAIL"; echo "$$out" | sed 's/^/                 /'; fail=1; fi; \
	printf '  py fragmpd     '; \
