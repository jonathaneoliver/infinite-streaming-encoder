...
# container and the sibling worker containers (ENCODER_IMAGE), so the two
# targets differ only in this one value:
...
.PHONY: require-paths build run run-remote stop restart logs shell status clean push push-setup cloud-push version
...
# selects the image for the server AND the sibling workers it spawns, so the
...
# ENCODER_IMAGE for the sibling workers); logs into GHCR first only if GHCR_PAT
# is set, which is needed when the package is private.
