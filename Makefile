...
# is PUBLIC, so workers pull with no auth — we deliberately do NOT forward
# GHCR_PAT to them: a `docker login` over SSH fails headlessly on a macOS box
# (Docker Desktop's keychain credsStore: "-25308 User interaction is not
# allowed"), which is what used to force the macmini onto the image-transfer
# path. Pair with `make run-remote` on the master for a fully no-local-build
# bring-up. (Forked to a PRIVATE package? Log each worker box into GHCR by hand
# first — not supported headlessly on macOS.)
dist-deploy-ghcr:     ## GHCR-pull workers on each DIST_WORKERS box (no build/transfer, no auth)
	@if [ -z "$(DIST_WORKERS)" ]; then echo "set DIST_WORKERS=label=ssh_target [..] (in .env)"; exit 1; fi
	@for w in $(DIST_WORKERS); do \
	  label=$${w%%=*}; host=$${w#*=}; \
	  MASTER_IP=$(MASTER_IP) IMAGE=$(REMOTE_IMAGE) GHCR_USERNAME=$(GHCR_USERNAME) \
