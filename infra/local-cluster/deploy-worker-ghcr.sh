...
# GHCR — no image transfer, no code rsync, no per-box build. The published
# image already carries both the encoder code and temporalio, so the box just
# pulls it and runs the worker. Works for any arch (GHCR is multi-arch). Run
# from the master (repo root). Idempotent — safe to re-run.
#
#   deploy-worker-ghcr.sh <ssh_target> <label>
#
# Env:
#   MASTER_IP            LAN IP the worker dials for Temporal + MinIO (required
#                        in practice; default 192.168.0.110)
#   IMAGE                GHCR ref to pull (default ghcr.io/jonathaneoliver/encoder:latest)
#   GHCR_PAT             set only if the GHCR package is private (piped to
#                        `docker login` over ssh stdin — never on the arg list)
